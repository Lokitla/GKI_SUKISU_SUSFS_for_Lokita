...
| `--rekernel-x` | Enable ReKernel-X (myflavor fork of Re-Kernel with async-txn dedup on by default; mutually exclusive with `--rekernel`, which wins if both are given) |
