...
**默认只构建 5.10 的单个子版本做冒烟**（走「Android 内核构建-自定义」，一次就 1 个构建），
不是一上来就跑满 80 个版本。理由有两条：全量构建会让 80 个任务同时去
`android.googlesource.com` 拉源码，对上游是实打实的压力；也容易把时长烧在一个
可能根本编不过的新提交上。冒烟通过后，再手动跑「构建内核」选全部版本。

> 冒烟目标刻意选 5.10 而不是子版本最少的 6.12：**SukiSU 主线目前与 6.12 不兼容**
> （`security_add_hooks` 第三参数在 6.12 上是 `const struct lsm_id *`，SukiSU 仍按老
> 签名传字符串），主线版本必然编不过，用它做冒烟只会永远飘红。6.12 请用老版本
> SukiSU（40726 / 40548）。
...
| `build_scope` | `单版本冒烟` / `全部版本` / `不构建` | `单版本冒烟` |
| `release_type` | `Release` / `Pre-Release` / `Actions` | `Release` |

> 选 `全部版本` 或 `不构建` 之外的值时走 `kernel-custom.yml`；选 `全部版本` 才走
> `main.yml` 展开完整矩阵。
